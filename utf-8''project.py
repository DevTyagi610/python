...
#finding bounding box for all pages
for img_name in parsed_img_src.keys() :
    open_cv_img = np.array(parsed_img_src[img_name]["pil_img"])
    img_gray = cv.cvtColor(open_cv_img , cv.COLOR_BGR2GRAY)
    faces_bounding_boxes = face_cascade.detectMultiScale(img_gray, 1.3, 5)
    ...
    for img_name in parsed_img_src :
        if (keyword in parsed_img_src[img_name]['text']) :
            if(len(parsed_img_src[img_name]['faces']) != 0) :
                print("results found in {}" .format(img_name))
